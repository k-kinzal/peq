    ...
    public static function process(FunctionLike $node, Scope $scope): array
    {
        ...
        if ($returnType instanceof Name) {
            ...
            if (!self::isBuiltin($typeName)) {
                ...
                $items[] = new DeclarationTypeReturnEdge($sourceNode, $typeNode, new FileMeta($scope->getFile(), $returnType->getStartLine(), 1));
            }
        }
        ...
        foreach ($node->getParams() as $param) {
            ...
            if ($param->type instanceof Name) {
                ...
                if (!self::isBuiltin($typeName)) {
                    $typeNode = new ClassNode(new ClassNodeId(self::getNamespace($typeName), self::getShortName($typeName)), false, null);
                    $items[] = new DeclarationTypeParameterEdge($sourceNode, $typeNode, new FileMeta($scope->getFile(), $param->type->getStartLine(), 1));
                }
            }
        }
