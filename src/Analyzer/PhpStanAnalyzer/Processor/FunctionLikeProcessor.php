...
final class FunctionLikeProcessor
{
     ...
     *
     * @return list<AttributeEdge|DeclarationMethodEdge|DeclarationTypeParameterEdge|DeclarationTypeReturnEdge|FunctionNode|MethodNode> The relations it describes
     */
    public static function process(FunctionLike $node, Scope $scope): array
    {
        ...
        if ($node instanceof ClassMethod) {
            ...
            return [
                new DeclarationMethodEdge(self::declaringNode($classReflection), $declared, $meta),
                $declared,
     ...
     *
     * @return list<AttributeEdge|DeclarationTypeParameterEdge|DeclarationTypeReturnEdge> The signature relations
     */
    public static function signature(FunctionLike $node, FunctionNode|MethodNode $declared, Scope $scope, FileMeta $meta): array
    {
        ...
        foreach (TypeResolver::references($node->getReturnType(), $scope->getFile()) as $type) {
            $items[] = new DeclarationTypeReturnEdge($declared, $type->node, $type->meta);
        }
        ...
        foreach ($node->getParams() as $param) {
            ...
            foreach (TypeResolver::references($param->type, $scope->getFile()) as $type) {
                $items[] = new DeclarationTypeParameterEdge($declared, $type->node, $type->meta);
            }
        }
