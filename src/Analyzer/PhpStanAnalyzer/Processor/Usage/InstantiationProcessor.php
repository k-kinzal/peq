<?php

declare(strict_types=1);

namespace App\Analyzer\PhpStanAnalyzer\Processor\Usage;

use App\Analyzer\Graph\Edge\Usage\InstantiationEdge;
use App\Analyzer\Graph\FileMeta;
use App\Analyzer\Graph\Node;
use App\Analyzer\Graph\Node\ClassNode;
use App\Analyzer\Graph\Node\FunctionNode;
use App\Analyzer\Graph\Node\MethodNode;
use App\Analyzer\Graph\NodeId\ClassNodeId;
use App\Analyzer\Graph\QualifiedName;
use App\Analyzer\PhpStanAnalyzer\SourceResolver;
use PhpParser\Node as PhpParserNode;
use PhpParser\Node\Expr\New_;
use PHPStan\Analyser\Scope;

/**
 * Records a class being instantiated.
 *
 * `new` names a class directly, which makes it one of the strongest relations in the
 * graph: the instantiating code cannot survive the class disappearing.
 *
 * @visibility App\Analyzer\PhpStanAnalyzer
 */
final class InstantiationProcessor
{
    /**
     * Records what this declaration or expression brings into the graph.
     *
     * @param New_      $node       The syntax node met during analysis
     * @param Scope     $scope      The analyser scope it was written in
     * @param null|Node $sourceNode The symbol it is written inside, resolved from the scope when omitted
     *
     * @return list<InstantiationEdge> The relations it describes
     */
    public static function process(New_ $node, Scope $scope, ?Node $sourceNode = null): array
    {
        $items = [];
        $sourceNode ??= SourceResolver::resolve($scope);

        if ($node->class instanceof PhpParserNode\Name) {
            $className = $scope->resolveName($node->class);
            if (!(new QualifiedName($className))->isBuiltinType()) {
                $targetNode = new ClassNode(
                    ClassNodeId::of($className),
                    false,
                    null
                );
                if ($sourceNode instanceof FunctionNode || $sourceNode instanceof MethodNode) {
                    $meta = new FileMeta($scope->getFile(), $node->getStartLine(), 1, $node->getStartFilePos());
                    $items[] = new InstantiationEdge($sourceNode, $targetNode, $meta);
                }
            }
        }

        return $items;
    }
}
