...
final class PatternMatchingTest extends TestCase
{
    ...
    {
        self::assertSame(
            'c=Controller m=Controller::show q=Kernel; c=Controller m=Controller::store q=Kernel',
            MatchedPattern::of('(c:Class)-[:declaresMethod]->(m), (c)-[:extends]->(q)'),
        );
    }
    ...
    {
        self::assertSame('', MatchedPattern::of('(c:Class)-[:declaresMethod]->(m), (c)-[:implements]->(q)'));
    }
    ...
    {
        $bound = ['n' => new IntegerDatum(1)];

        self::assertSame('n=1 p=Missing', MatchedPattern::of('(p:Unresolved)', $bound));
    }
