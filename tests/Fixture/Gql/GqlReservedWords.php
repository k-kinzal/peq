 ...
 *
 * The reference's table is not complete: `DAY`, `HOUR`, `IMPLIES`, `MINUTE`, `MONTH`,
 * `SECOND` and `YEAR` are `<reserved word>` in ISO's published grammar artifact, and
 * `WHITESPACE` is `<pre-reserved word>`, though the page prints none of the eight. They
 * are marked here by the production that holds them, because where the two disagree the
 * standard decides. The artifact is checked in at `spec/iso/gql.bnf.xml` under the
 * checksum ISO publishes it with, and `composer spec` compares it to what peq reserves.
 *
 * @see https://learn.microsoft.com/fabric/graph/gql-reference-reserved-terms GQL reserved words reference
 * @see spec/iso/gql.bnf.xml ISO/IEC 39075's published grammar artifact
 */
