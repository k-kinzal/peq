 ...
 * line something is written on, and a formatter would quietly take that away.
 */
final class EquivalenceCorpus
{
    /**
     * Returns every scenario the two engines are checked on.
     *
     * @return array<string, array<string, string>> The files of each scenario, by scenario name
     */
    public static function scenarios(): array
    {
        return array_merge(self::declarationScenarios(), self::usageScenarios(), self::traitScenarios(), self::edgeCaseScenarios());
    }

    /**
     * Returns the scenarios about what a codebase declares.
     *
     * @return array<string, array<string, string>> The files of each scenario, by scenario name
     */
    public static function declarationScenarios(): array
    {
        return [
    ...
    /**
     * Returns the scenarios about what a codebase reaches out to.
     *
     * @return array<string, array<string, string>> The files of each scenario, by scenario name
     */
    public static function usageScenarios(): array
    {
        return [
    ...
    /**
     * Returns the scenarios about the members a class takes on from a trait.
     *
     * @return array<string, array<string, string>> The files of each scenario, by scenario name
     */
    public static function traitScenarios(): array
    {
        return [
    ...
    /**
     * Returns the scenarios about what neither engine is asked about every day.
     *
     * @return array<string, array<string, string>> The files of each scenario, by scenario name
     */
    public static function edgeCaseScenarios(): array
    {
        return [
    ...
    /**
     * Returns the graph of every scenario at once.
     *
     * What the corpus exercises is a question about the corpus as a whole: a kind of
     * symbol no scenario declares is a kind neither engine is ever checked on, and
     * that is worth knowing whichever scenario was supposed to cover it.
     *
     * @return Graph The graph of every scenario
     *
     * @throws RuntimeException If a scenario cannot be written
     */
    public static function graph(): Graph
    {
        $graph = new Graph();
        foreach (self::scenarios() as $scenario => $files) {
            $graph = $graph->merge((new NativeAnalyzer())->analyze(self::writeTo($scenario, $files)));
        }
    ...
    /**
     * Writes one scenario out and returns the directory it was written to.
     *
     * @param string                $name  The name of the scenario
     * @param array<string, string> $files Its files, by file name
     *
     * @return string The directory holding that scenario
     *
     * @throws RuntimeException If the scenario cannot be written
     */
    public static function writeTo(string $name, array $files): string
    {
        $directory = sys_get_temp_dir().'/peq-corpus-'.md5($name.serialize($files));
        if (!is_dir($directory) && !mkdir($directory, 0o777, true) && !is_dir($directory)) {
            throw new RuntimeException(sprintf('Unable to create the directory "%s"', $directory));
        }

        foreach ($files as $file => $contents) {
            if (file_put_contents($directory.'/'.$file, $contents) === false) {
